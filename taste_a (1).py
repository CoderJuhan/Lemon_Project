...
section[data-testid="stSidebar"]{
  ...
  border-right:1px solid rgba(255,79,163,0.35) !important;
}
section[data-testid="stSidebar"] *{
...
section[data-testid="stSidebar"] button:hover{
  border: 1px solid rgba(255,79,163,0.55) !important;
}
...
@keyframes sparkle{
  0%{filter: drop-shadow(0 0 0px #ff4fa3);}
  50%{filter: drop-shadow(0 0 16px #ff4fa3);}
  100%{filter: drop-shadow(0 0 0px #ff4fa3);}
}
.noti-wrap img:active{
  animation: jump 0.6s ease, sparkle 0.6s ease;
}
...
.noti-text strong{ color:#ffffff !important; }
.noti-text span{ color:rgba(243,246,255,0.82) !important; }
</style>
...
def insert_photo(pid, store_name, date_str, image_path):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT INTO photos (id, store_name, date, image_path) VALUES (?, ?, ?, ?)",
...
def get_photos_by_date(date_str):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, store_name, date, image_path FROM photos WHERE date = ? ORDER BY rowid ASC",
              (date_str,))
    rows = c.fetchall()
    conn.close()
    return rows
...
def noti_img_html():
    # noti_pin.png를 base64로 넣어서 "무조건" 뜨게 함
    p = Path("noti_pin.png")
    if not p.exists():
        return "<div class='noti-text'><strong>noti_pin.png</strong><br/><span>파일이 app.py 옆에 있어야 해요</span></div>"
    b = p.read_bytes()
    encoded = base64.b64encode(b).decode("utf-8")
    return f"<div class='noti-wrap'><img src='data:image/png;base64,{encoded}'/></div>"
...
# =========================
# 사이드바: 노티 + 필터 + 화면 이동 + 날짜(선택 즉시 앨범 이동)
# =========================
def go_album():
...
with st.sidebar:
    st.markdown(noti_img_html(), unsafe_allow_html=True)
    st.markdown("""
    <div class="noti-text">
        <strong>오늘의 기록을 남겨보세요</strong><br/>
        <span>맛집 · 리뷰 · 사진은 전부 자동 저장돼요</span>
    </div>
    """, unsafe_allow_html=True)
...
st.sidebar.date_input(
    "날짜 선택",
    value=st.session_state["album_date"],
    key="album_date",
    on_change=go_album,   # ✅ 날짜 누르면 자동으로 앨범 화면으로 이동
)
...
# =========================
    ...
    if q.strip():
        qq = q.strip().lower()
        rows = [r for r in rows if (r[1] and qq in r[1].lower()) or (r[2] and qq in r[2].lower())]
...
# =========================
    ...
    colors = [color_map.get(l, "#BAB0AC") for l in labels]

    # ---------- 차트 (글씨 작게 고정) ----------
    fig, ax = plt.subplots(figsize=(4.4, 2.8), dpi=180)
    ...
    plt.subplots_adjust(top=0.75, bottom=0.32)
